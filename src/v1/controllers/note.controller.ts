...
import { NoteCommentModel } from "../models/notecomment";
...
export const updateNoteContent = async (req: MyRequest, res: Response) => {
	const t = await sequelize.transaction();
	try {
		const body = req.body;
		...
		if (newPermission === "view" || newPermission === "none") {
			throw new ApiError(403, "You do not have permission to access this note");
		}
		...
		await Promise.all([
			Note.update({ content }, { where: { id: note_id }, transaction: t }),
		]);
...
export const getDetailNote = async (req: MyRequest, res: Response) => {
	...
	const note_threads = await NoteThread.findAll({
		where: {
			note_id: existNote.id,
		},
	});

	const note_threads_ids = note_threads.map((nt) => nt.id);
	const note_comments = await NoteComment.findAll({
		where: {
			note_thread_id: {
				[Op.in]: note_threads_ids,
			},
		},
	});

	if (note_comments.length) {
		const thread_map = new Map<number, NoteCommentModel[]>();

		for (const comment of note_comments) {
			comment["id"] = comment.id.toString();
			comment["type"] = "comment";
			comment["timestamp"] = new Date(comment.createdAt).getTime();
			const comments = thread_map.get(comment.note_thread_id) || [];
			comments.push(comment);
			thread_map.set(comment.note_thread_id, comments);
		}

		for (const thread of note_threads) {
			const comments = thread_map.get(Number(thread.id)) || [];
	...
	if (!user_id || existNote.user_id !== user_id) {
		...
		if (existMember instanceof ApiError) {
			...
			if (folder.is_in_teamspace) {
				...
				//HERE: status is public or shared or workspace
				is_guest = true;
...
export const createNewThread = async (req: MyRequest, res: Response) => {
	const t = await sequelize.transaction();
	try {
		const body = req.body;
		const note_id = Number(req.params.note_id || 0);
		...
		if (!thread || !comment) {
			throw new ApiError(400, "Thread and comment default are required");
		}

		const newThread = await NoteThread.create(
			{
				note_id: note_id,
				quote: thread.quote || "",
				ref_author_id: Number(thread.ref_author_id || 0),
			},
			{ transaction: t }
		);
		...
		const newComment = await NoteComment.create(
			{
				note_thread_id: newThread.id,
				author: comment.author || "Anonymous",
				content: comment.content || "",
				ref_author_id: Number(comment.ref_author_id || 0),
			},
			{ transaction: t }
		);
		await t.commit();
...
export const createComment = async (req: MyRequest, res: Response) => {
	...
	const newComment = await NoteComment.create({
		note_thread_id: Number(body.note_thread_id || 0),
		author: body.author || "Anonymous",
		content: body.content || "",
		ref_author_id: Number(body.ref_author_id || 0),
	});
...
export const deleteComment = async (req: MyRequest, res: Response) => {
	try {
		const comment_id = Number(req.params.comment_id || 0);
		const note_id = Number(req.params.note_id || 0);

		const author_id = Number(req.query.author_id || 0);

		const user_id = req.user_id;

		const existComment = await NoteComment.findByPk(comment_id);
		...
		if (existComment.ref_author_id === author_id) {
			is_access = true;
		}
...
export const deleteThread = async (req: MyRequest, res: Response) => {
	const t = await sequelize.transaction();
	try {
		const thread_id = Number(req.params.thread_id || 0);
		const note_id = Number(req.params.note_id || 0);
		...
		const existThread = await NoteThread.findByPk(thread_id);
		if (!existThread) {
		...
		if (existThread.ref_author_id === author_id) {
			is_access = true;
		}
