...
export const getAllWorkspaces = async (req: MyRequest, res: Response) => {
	...
	const result = await getAllWorkspaceOfUser(owner_id);
	...
	const {} = result;
	const { workspaces, workspacesBeInvited } = result;

	let pos = 0;
	for (const w of workspaces) {
		w["is_guest"] = false;
		w["member_count"] = Number(w["member_count"]) || 1;
		w["pos"] = pos++;
	}
...
export const createNewWorkspace = async (req: MyRequest, res: Response) => {
	...
	const allWorkspace = await getAllWorkspaceOfUser(owner_id);

	if (!(allWorkspace instanceof ApiError)) {
		let pos = 0;
		const { workspaces, workspacesBeInvited } = allWorkspace;
		for (const w of workspaces) {
			w["pos"] = pos++;
		}
		for (const w of workspacesBeInvited) {
...
async function getAllWorkspaceOfUser(owner_id: number) {
	try {
		const query = `
  ...
  WHERE np.user_id = :user_id AND ws.id NOT IN (:w_ids) AND nt.status = 'shared' AND nt.deleted = false`;
		...
		return { workspaces, workspacesBeInvited };
	} catch (error) {
		return new ApiError(
			error.statusCode || 500,
			error.message || "Internal server error"
