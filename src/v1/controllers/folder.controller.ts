...
import { isProduction, log_action } from "../../../utils/utils";
import {
	defaultMessageError,
	startingNoteContent,
} from "../../../utils/contants";
...
export const deleteFolder = async (req: MyRequest, res: Response) => {
	...
	try {
		let error = null;
		async function deleteChild(id: number, user_id: number) {
			try {
				if (error) {
					return error;
				}
				...
				childs.forEach(async (child) => {
					...
					promises.push(deleteChild(child.id, user_id));
				});
		...
		const bind = await deleteChild(folder_id, exist.user_id);
		...
		try {
			...
			throw new ApiError(
				...
				error.message || error || defaultMessageError
			);
		}
...
export const createRootFolderAndNoteInTeamspaceDefault = async (
	...
	try {
		...
		const exist = await getRootFolderInTeamspace(user.id, workspace_id);

		if (exist && !(exist instanceof ApiError)) {
			throw new ApiError(400, "Root folder in teamspace already exists");
		}
